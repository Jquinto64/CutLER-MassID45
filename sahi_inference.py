...
def main(args):
    ...
    model_ids = args.model_path.split('/')[-2].split("_")
    model_id_from_folder = "_".join(model_ids[1:])
    model_id = args.exp_name or model_id_from_folder
    exp_name = f"exp_{postprocess_match_threshold_id}_{model_conf_threshold}_conf_{model_id}"

    if os.path.exists(f"/scratch/ssd004/scratch/jquinto/CutLER/runs/predict/{exp_name}"):
    ...
    if args.super_resolution is True and scale_factor > 1:    
        ...
        with open(f"/scratch/ssd004/scratch/jquinto/CutLER/runs/predict/{exp_name}/result.json", "r") as f:
        ...
        with open(f"/scratch/ssd004/scratch/jquinto/CutLER/runs/predict/{exp_name}/result.json", "w") as f:
    ...
    for det_type in ["bbox", "segm"]:
        evaluate(
            ...
            result_json_path=f"/scratch/ssd004/scratch/jquinto/CutLER/runs/predict/{exp_name}/result.json",
            out_dir=f"/scratch/ssd004/scratch/jquinto/CutLER/runs/predict/{exp_name}",
            type=det_type,
            classwise=True,
            max_detections=1000,
            areas = areas
